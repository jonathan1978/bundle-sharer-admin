      ...
      Http.get('customers/approved')
        .then((response) => {
          ...
          response.forEach(item => data.push({
            ...
            email: item.email,
            status: <Badge className={"font-size-12 badge-soft-" + item.status === 'approved' ? 'success' : 'danger'} color={item.status === 'approved' ? 'success' : 'warning'} pill>{item.status}</Badge>,
            telephone: item.telephone,
            businessCertificate: item.businessCertificate,
            action: <Button type="button" style={{backgroundColor: "#e60000", borderColor: "#e60000", color: "#ffffff"}} className="btn-sm btn-rounded" onClick={() => handleActivate(item.id)} >Account</Button>
          }));
          console.log(data);
          setRow(data);
        })
